package com.spotcamp.module.publicmarket.controller;
...
import com.spotcamp.module.publicmarket.service.PublicCampsiteService;
import com.spotcamp.module.visualmap.dto.MapSummaryResponseDTO;
...
public class PublicController {
    ...
    @ApiResponses(value = {
        @ApiResponse(
            ...
            content = @Content(
                ...
                schema = @Schema(implementation = CampsitePublicResponseDTO.class)
            )
        )
    })
    @GetMapping("/campsites")
    public ResponseEntity<List<CampsitePublicResponseDTO>> searchCampsites(
        ...
        Page<CampsitePublicResponseDTO> result = publicCampsiteService.searchCampsites(query, page, limit);
    ...
    @ApiResponses(value = {
        @ApiResponse(
            ...
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = CampsitePublicDetailDTO.class)
            )
        ),
        @ApiResponse(responseCode = "404", description = "Campsite not found")
    })
    @GetMapping("/campsites/{campsiteId}")
    public ResponseEntity<CampsitePublicDetailDTO> getCampsiteDetail(
            @Parameter(description = "Campsite ID")
            @PathVariable Long campsiteId
    ) {
        log.debug("Get campsite detail: {}", campsiteId);

        CampsitePublicDetailDTO detail = publicCampsiteService.getCampsiteDetail(campsiteId);
    ...
    @ApiResponses(value = {
        @ApiResponse(
            ...
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = MapConfigPublicResponseDTO.class)
            )
        ),
        @ApiResponse(responseCode = "404", description = "Map configuration not found")
    })
    @GetMapping("/maps/{campsiteId}/config")
    public ResponseEntity<MapConfigPublicResponseDTO> getMapConfig(
            @Parameter(description = "Campsite ID")
            @PathVariable Long campsiteId,
        ...
        MapConfigPublicResponseDTO config = publicCampsiteService.getMapConfig(campsiteId, mapCode, mapId);
        if (config == null) {
    ...
    public ResponseEntity<MapConfigPublicResponseDTO> getMapConfigById(
            @Parameter(description = "Campsite ID")
            @PathVariable Long campsiteId,
        ...
        MapConfigPublicResponseDTO config = publicCampsiteService.getMapConfig(campsiteId, mapCode, null);
        if (config == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(config);
    ...
    public ResponseEntity<List<MapSummaryResponseDTO>> listMapConfigs(
        ...
        List<MapSummaryResponseDTO> response = publicCampsiteService.getActiveMapConfigs(campsiteId)
                .stream()
                .map(config -> MapSummaryResponseDTO.builder()
                        .id(config.getId())
    ...
    private List<Long> extractProductIds(com.spotcamp.module.visualmap.entity.MapConfiguration config) {
        if (config.getConfigData() == null || config.getConfigData().getSpots() == null) {
            return List.of();
        }
        return config.getConfigData().getSpots().stream()
                .map(com.spotcamp.module.visualmap.entity.SpotDefinition::getProductId)
